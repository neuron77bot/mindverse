...
export async function transcriptionRoutes(app: FastifyInstance) {
    ...
    async (req, reply) => {
      try {
        ...
        if (!data) {
          return reply
            .status(400)
            .send({ success: false, error: 'No se recibió archivo de audio' });
        }
  ...
  app.post<{ Body: { step: string; actions: string[]; context?: string } }>(
    '/refine-step',
    {
      schema: {
        tags: ['transcription'],
        ...
        body: {
          ...
          properties: {
            ...
            actions: {
              type: 'array',
              items: { type: 'string' },
              description: 'Acciones actuales del paso',
            },
