...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.get('/', {
    schema: {
      ...
      response: {
        200: { type: 'object', properties: { success: { type: 'boolean' }, data: { type: 'array', items: userShape } } },
      },
    },
  }, async (_req, reply) => {
    try {
  ...
  app.get<{ Params: { googleId: string } }>('/:googleId', {
    schema: {
      tags: ['users'],
  ...
  app.patch<{ Params: { googleId: string }; Body: Record<string, any> }>('/:googleId', {
    schema: {
      tags: ['users'],
  ...
  app.delete<{ Params: { googleId: string } }>('/:googleId', {
    schema: {
      tags: ['users'],
