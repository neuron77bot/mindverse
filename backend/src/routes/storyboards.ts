...
export async function storyboardRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { title: string; originalText: string; inputMode: string; frames: any[]; mermaidDiagram?: string } }>('/', {
    schema: {
      tags: ['storyboards'],
  ...
  app.patch<{ Params: { id: string }; Body: { title?: string; frames?: any[]; comicPageUrl?: string; comicPagePrompt?: string } }>('/:id', {
    schema: {
      tags: ['storyboards'],
