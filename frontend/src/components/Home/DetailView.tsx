...
function NodeLabels({ node, size = 'md' }: { node: MindverseNode; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <span
        className={`inline-flex items-center gap-0.5 rounded-full bg-slate-700/60 text-slate-300 font-medium ${cls}`}
      >
...
export default function DetailView({ node, onBack, onNavigateToMap }: DetailViewProps) {
  ...
  const mermaidCode = useMemo(() => {
    ...
    steps.forEach((step, idx) => {
      const stepColor =
        EMOTIONAL_COLORS[step.emotionalLevel] || CATEGORY_COLORS[step.category] || '#6366F1';
    ...
    relevantConns.forEach((conn) => {
      const sourceLabel =
        conn.source === node.id ? 'ROOT' : `N${steps.findIndex((s) => s.id === conn.source)}`;
      const targetLabel =
        conn.target === node.id ? 'ROOT' : `N${steps.findIndex((s) => s.id === conn.target)}`;
      if (
        sourceLabel &&
        targetLabel &&
        !sourceLabel.includes('-1') &&
        !targetLabel.includes('-1')
      ) {
    ...
    <div className="flex-1 overflow-y-auto flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto lg:overflow-hidden">
        <div className="lg:h-full lg:grid lg:grid-cols-[380px_1fr] xl:grid-cols-[420px_1fr]">
          ...
          <div className="lg:overflow-y-auto lg:border-r" style={{ borderColor: `${vibColor}20` }}>
            {/* Imagen hero */}
            <div className="relative w-full overflow-hidden aspect-square shrink-0">
              {node.imageUrl ? (
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${vibColor}30 0%, ${vibColor}10 100%)`,
                  }}
                >
            ...
            {steps.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-700 p-12 text-center flex flex-col items-center">
                ...
                <p className="text-slate-600 text-sm mb-5">
                  Agregá el primer paso para este pensamiento
                </p>
                <button
                  onClick={() => openEditor(undefined, node.id)}
              ...
              <div className="space-y-2">
                {steps.map((step, idx) => {
                  const stepVibColor =
                    EMOTIONAL_COLORS[step.emotionalLevel] ||
                  ...
                  return (
                    <div
                      ...
                      style={{
                        backgroundColor: `${stepVibColor}08`,
                        borderColor: `${stepVibColor}25`,
                      }}
                      ...
                      <div className="relative shrink-0 w-12 h-12 rounded overflow-hidden">
                        {step.imageUrl ? (
                          ...
                          <div
                            className="w-full h-full flex items-center justify-center"
                            style={{
                              background: `linear-gradient(135deg, ${stepVibColor}25 0%, ${stepVibColor}10 100%)`,
                            }}
                          >
                ...
                {steps.map((step, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        backgroundColor: `${stepVibColor}0d`,
                        borderColor: `${stepVibColor}30`,
                      }}
                      onClick={() => openEditor(step)}
                    >
                      ...
                      {/* Contenido */}
                      <div className="p-4">
                        <p className="text-white font-semibold leading-tight mb-2 text-sm">
                          {step.content}
                        </p>
                        <NodeLabels node={step} size="sm" />
                        {step.description && (
