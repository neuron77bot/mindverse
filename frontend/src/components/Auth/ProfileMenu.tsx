...
import { logout } from './LoginView';

interface ProfileMenuProps {
  onLogout: () => void;
}

const USERNAME = 'Matius';
...
export default function ProfileMenu({ onLogout }: ProfileMenuProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-indigo-500/25 shrink-0">
          {USERNAME[0]}
        </div>
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute right-0 top-full mt-2 w-52 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl shadow-black/40 overflow-hidden z-50">
          {/* Info usuario */}
          <div className="px-4 py-3 border-b border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs">Mindverse</p>
              </div>
            </div>
          </div>
