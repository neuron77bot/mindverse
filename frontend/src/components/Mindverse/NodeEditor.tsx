...
export default function NodeEditor() {
  ...
  const handleSave = () => {
    ...
    if (selectedNode) {
      ...
      const currentInIds = connections
        .filter((c) => c.target === selectedNode.id)
        .map((c) => c.source);
      const currentOutIds = connections
        .filter((c) => c.source === selectedNode.id)
        .map((c) => c.target);

      const inChanged =
        JSON.stringify([...inNodeIds].sort()) !== JSON.stringify([...currentInIds].sort());
      const outChanged =
        JSON.stringify([...outNodeIds].sort()) !== JSON.stringify([...currentOutIds].sort());
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setInNodeIds(inNodeIds.filter((id) => id !== nodeId))
                                    }
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    setOutNodeIds(outNodeIds.filter((id) => id !== nodeId))
                                  }
                                  className="hover:text-red-400 transition-colors"
                                >
                                  ✕
